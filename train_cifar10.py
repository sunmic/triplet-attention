...
def main():
    ...
    # create model
    if args.arch == "resnet":
        model = ResidualNet("CIFAR10", args.depth, NUM_CLASSES, args.att_type)
    elif args.arch == 'resnet_plus1':
        model = ResidualNetPlus1('CIFAR10', args.depth, NUM_CLASSES, args.att_type)
    ...
    task = Task.init(project_name='attention-explanation',
                     task_name=f'PCAMResNet{args.depth} training on CIFAR10 dataset')
...
def save_checkpoint(state, is_best, prefix, epoch):
    ...
    filename = "./checkpoints/%s_checkpoint_resume.pth.tar" % (prefix)
    torch.save(state, filename)
    if is_best:
